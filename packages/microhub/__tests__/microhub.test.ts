import { createMicrohub } from "../src/microhub";

describe("@zodash/microhub", () => {
  it('common function service', () => {
    type FunctionService = (number1: number, number2: number) => any;

    const microhub = createMicrohub<FunctionService>();
    ...
    expect(microhub.discover('multiple')).toBeUndefined();
  ...
  it('component service', () => {
    ...
    const microhub = createMicrohub<Component<Props>>();
