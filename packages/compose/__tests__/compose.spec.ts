import { compose, Middleware, Next } from '../src/compose';
...
describe('@zodash/compose', () => {
  it('middleware', () => {
    ...
    class App {
      ...
      private _handler: Middleware<Context> | null = null;
    ...
    const m1 = async (context: Context, next: Next) => {
      context.state.push(1);
      await next();
      context.state.push(6);
    };

    const m2 = async (context: Context, next: Next) => {
      context.state.push(2);
      await next();
      context.state.push(5);
    };

    const m3 = async (context: Context, next: Next) => {
