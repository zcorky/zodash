 ...
 * @returns utf8 string
 */
export function transformUnicodeString2Utf8String(
  ...
  return unicodeString
    ...
    .reduce((all, char) => {
      ...
      } else if (charCode < 0b000100000000000) {
        // UTF8 - 2 个字节
...
export function transformUtf8String2UnicodeString(utf8String: string) {
  ...
  while (i < utf8String.length) {
    ...
    if (c1 < 0b10000000) {
      ...
      charCode = c1;
      i += 1;
    } else if (c1 >= 0b11000000 && c1 < 0b11100000) {
      // 2 个字节
      ...
      i += 2;
    } else if (c1 >= 0b11100000 && c1 < 0b11110000) {
      // 3 个字节
      const c2 = utf8String.charCodeAt(i + 1);
 ...
 */
export function transformUnicodeString2Utf8Bytes(
  unicodeString: string,
): Byte[] {
  const bytes: Byte[] = [];
  return unicodeString
...
export function bytes2String(bytes: Byte[]) {
  return bytes.map((charCode) => String.fromCharCode(charCode)).join('');
}
